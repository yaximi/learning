...
如果你是一名 JavaScript 开发者，或者想要成为一名 JavaScript 开发者，那么你必须知道 JavaScript 程序内部的执行机制。
理解执行上下文和执行栈同样有助于理解其他的 JavaScript 概念如提升机制、作用域和闭包等。
...
3、执行栈
    ...
    当 JavaScript 引擎首次读取你的脚本时，它会创建一个全局执行上下文并将其压入当前的执行栈。
    每当发生一个函数调用，引擎都会为该函数创建一个新的执行上下文并将其压到当前执行栈的顶端。
...
4、执行上下文是如何被创建的
    ...
    创建阶段：在任意的 JavaScript 代码被执行前，执行上下文处于创建阶段。
    ...
    在词法环境中，有两个组成部分：
        ...
        2）对外部环境的引用
    词法环境有两种类型：
        1）全局环境（在全局执行上下文中）是一个没有外部环境的词法环境。全局环境的外部环境引用为 null。它拥有一个全局对象（window 对象）及其关联的方法和属性（例如数组方法）以及任何用户自定义的全局变量，this 的值指向这个全局对象。
    ...
    环境记录 同样有两种类型（如下所示）：
        1）声明性环境记录（Declarative）存储变量、函数和参数。一个函数环境包含声明性环境记录。
        2）对象环境记录（Object）用于定义在全局执行上下文中出现的变量和函数的关联。全局环境包含对象环境记录。
    ...
    执行阶段：这是整篇文章中最简单的部分。在此阶段，完成对所有变量的分配，最后执行代码。
